for animalNumber = 1:4
  ...
  %      filename = 'D:\Users\Bailey\Documents\Dropbox\Mouse MEA\Mouse MEA\Mat\25\bintest03_2014-04-19 m25rrri-01_sorted.txt';
        ...
        clearvars numtest totcount numchan numcell;
        A_LL_changecell_RfDel{animalNumber,dayNumber}(:,1) = cellfun (@(x) length(cat(2,x{goodLL_TS & data.LL_Rf_Mask}))/length(find(goodLL_TS & data.LL_Rf_Mask)), LLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
        A_LL_changecell_RfDel{animalNumber,dayNumber}(:,2) = cellfun (@(x) length(cat(2,x{goodLL_TS & ~data.LL_Rf_Mask & data.burstInfo.LL_Last}))/length(find(goodLL_TS & ~data.LL_Rf_Mask & data.burstInfo.LL_Last)), LLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
        A_RL_changecell_RfDel{animalNumber,dayNumber}(:,1) = cellfun (@(x) length(cat(2,x{goodRL_TS & data.RL_Rf_Mask}))/length(find(goodRL_TS & data.RL_Rf_Mask)), RLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
        A_RL_changecell_RfDel{animalNumber,dayNumber}(:,2) = cellfun (@(x) length(cat(2,x{goodRL_TS & ~data.RL_Rf_Mask & data.burstInfo.RL_Last}))/length(find(goodRL_TS & ~data.RL_Rf_Mask & data.burstInfo.RL_Last)), RLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
        %
        % %%
    end
end
