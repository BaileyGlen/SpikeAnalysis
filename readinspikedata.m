...
%    LLSumArray2{y,1} = arrayfun (@(x) cat(2, LLSumArray{y,1}, celltest{y}(celltest{y}> (x) & celltest{y}< (x+2))-x),data.LL_TS,'UniformOutput', false);
    ...
    RLSumArray1{y,1} = arrayfun (@(x) cat(2, RLSumArray{y,1}, celltest{y}(celltest{y}> (x+2) & celltest{y}< (x+1.5))-x),data.RL_TS ,'UniformOutput', false);
...
clearvars numtest totcount numchan numcell;
A_LL_changecell{8}(:,1) = cellfun (@(x) length(cat(2,x{goodLL_TS & data.LL_Rf_Mask}))/length(find(goodLL_TS & data.LL_Rf_Mask)), LLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
A_LL_changecell{8}(:,2) = cellfun (@(x) length(cat(2,x{goodLL_TS & ~data.LL_Rf_Mask & data.burstInfo.LL_Last}))/length(find(goodLL_TS & ~data.LL_Rf_Mask & data.burstInfo.LL_Last)), LLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
A_RL_changecell{8}(:,1) = cellfun (@(x) length(cat(2,x{goodRL_TS & data.RL_Rf_Mask}))/length(find(goodRL_TS & data.RL_Rf_Mask)), RLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
A_RL_changecell{8}(:,2) = cellfun (@(x) length(cat(2,x{goodRL_TS & ~data.RL_Rf_Mask & data.burstInfo.RL_Last}))/length(find(goodRL_TS & ~data.RL_Rf_Mask & data.burstInfo.RL_Last)), RLSum1) ./ cellfun(@(y) length(y)/((length(mask)/400)),celltest   )/1.5;
% 
% %%
